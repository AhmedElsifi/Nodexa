...
export default function SectionCard({ section }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mt-xs grow">
        <h2 className="font-headline-md text-headline-md leading-8 font-semibold text-on-surface">
          {title}
        </h2>
        <p className="font-sans text-body-md leading-[24px] text-on-surface-variant mt-base line-clamp-2">
          {description}
        </p>
      </div>
      <div className="flex items-center gap-md mt-sm relative z-10 border-t border-outline-variant/50 pt-sm">
        <div className="flex items-center gap-base text-on-surface-variant">
          <span className="material-symbols-outlined text-body-lg">task</span>
          <span className="font-code text-label-sm leading-[16px] tracking-wider">
            {taskCount} Tasks
          </span>
        </div>
        <div className="flex items-center gap-base text-on-surface-variant">
      ...
      <div className="flex flex-col gap-base relative z-10 mt-sm">
        <div className="flex justify-between items-center">
          <span className="font-code text-label-sm leading-[16px] tracking-wider text-on-surface-variant">
            Progress
          </span>
          <span
            className={`font-code text-label-sm leading-[16px] tracking-wider ${
              progress === 100
                ? "text-primary"
                : progress > 0
                  ? "text-secondary"
      ...
      <Link
        to={`/sections/${id}`}
        className={`mt-sm relative z-10 w-full py-sm px-sm rounded-lg font-code text-label-sm leading-[16px] tracking-wider transition-colors flex justify-center items-center gap-xs ${
          isCompleted
